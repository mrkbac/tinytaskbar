...
| M22 | Normal, empty, and 120-window fixtures | Exercise all label, density, and ordering combinations with wheel and trackpad | Icon-only keeps labels/tooltips; compact does not clip; grouping is stable; wheel scrolls only on overflow; no visible/space-consuming scroller | Partial — automated full combination/geometry/order pass and live grouped compact icon-only plus overflow pass; physical trackpad sweep pending |
