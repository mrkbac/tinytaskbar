...
| M10a | AppKit app with several native tabs | Hover its single taskbar item, select each tab from the card, run Minimize All, then restore/select another tab | The card lists every native tab; selection targets that tab; minimizing never expands the group into separate taskbar items | Automated AX-group membership/projection/selection pass; signed synthetic visible-state AX ingestion pass; real Ghostty pass pending |
