...
| Seam | Responsibility |
| --- | --- |
| `WindowEligibility` | running regular app, AX role/subrole, and usable-size policy |
| `CGWindowMatcher` | exact numeric identity or conservative PID/layer/on-screen/bounds/title match |
...
| `TaskbarPresentationBuilder` | filters exclusions and applies pinned, ordered, and multi-display presentation policy |
| `TinyTaskbarPreferencesStore` | typed defaults, lossy record persistence, and pin/exclusion conflict rules |
...
`WindowSnapshotProvider`, and `ApplicationLaunching` seams. Production implementations call AX and
Core Graphics/AppKit APIs. The single private `_AXUIElementGetWindow` bridge is isolated
at the snapshot boundary; Space management remains on public APIs. Tests inject mock providers to prove denied startup does
...
Standard window close only closes the guide; the app
...
explicitly framed 620 × 640 point content view. Its content min/max sizes are both set
...
Rows are grouped under native General, Taskbar, and Applications section labels. Each
row uses the same content width and leading alignment, so Granted/Required
accessory-width changes cannot shift row labels. Action accessories retain their
intrinsic button/control widths, and the launch-at-login status is constrained to 190
points and at most two wrapped lines. There are no custom footer, Done, or Quit controls;
settings apply immediately and the standard close button completes onboarding. After
the first WindowServer order, one main-actor yield reasserts and
...
TinyTaskbar Dock icon while Settings is visible; the close callback restores
`.accessory`, deactivates the app, and removes that temporary Dock presence. Taskbar
panels never use `.regular` and remain non-activating.
...
window-title/application-name/icon-only labels, narrow/balanced/wide button widths,
shrink-then-scroll/automatic-icon overflow, standard/compact bar size, and three
multi-display modes. A
separate sheet manages pinned and excluded application records without normally
displaying paths. Invalid or absent typed values use current defaults. The current
`TaskbarState` is rerendered immediately without re-enumerating windows.
...
accepted by that subrole rule. Malformed frames and surfaces smaller than 80 × 40
points are excluded. Minimized and hidden AX windows remain eligible so their taskbar
item can restore or unhide them. That
...
from `NSScreen`. `_AXUIElementGetWindow` supplies an exact numeric identity when the
running macOS version supports it. Otherwise, the unchanged AX frame must match a
layer-zero, on-screen Core Graphics window for the same PID. Bounds are compared
within four points, with title matching preferred when both titles are available. A
unique bounds match remains valid when AX and CG title updates race; multiple
equal-bounds candidates still require title disambiguation. The matched CG window
number is stored as physical identity; the generated AX element identity remains the
actionable item key, with PID/title/geometry available as the synthetic fallback.
...
`CGWindowListCopyWindowInfo(.optionAll, .excludeDesktopElements, ...)` supplies both
on-screen and minimized physical records. Normal candidates must still match a record
whose `kCGWindowIsOnscreen` value is true, so other-Space windows and Stage Manager
background sets remain omitted. An AX candidate explicitly marked minimized may match
an off-screen record only by its exact `_AXUIElementGetWindow` number; title/frame
matching never promotes an ambiguous off-screen candidate. This includes windows that
were already minimized when TinyTaskbar started. Application-level hiding instead
retains only windows with prior positive on-screen evidence. This avoids promoting
background native tabs or other-Space records merely because their application is
...
The process-wide AX messaging timeout is 250 ms, bounding each synchronous
...
Activation first asks a hidden owning `NSRunningApplication` to unhide, then
defensively tries to clear `AXMinimized`, activates the owning
`NSRunningApplication`, sets `AXMain` and `AXFocused` when supported, and performs
`AXRaise`. A click refresh resolves an AX identity turnover through the unique physical
`(PID, CG window number)` identity. The provider keeps the same unique physical lookup
for an eligible AX element during a temporary AX/Core Graphics mismatch; ambiguous
physical aliases are never chosen. All operations tolerate stale references. The
resulting notification path schedules a refresh.
...
are rebuilt from current retained state and expose restore/minimize, Minimize All,
minimize others, and Close directly. Close is the final item nearest the bottom taskbar
edge. Pin/unpin, exclude, Settings, and Quit are isolated in a TinyTaskbar submenu. The
Close command reads the window's public
...
lead each display as normal window buttons or an icon-only launcher. The default
multi-display policy keeps each window on the taskbar belonging to its owning display.
The alternatives mirror every current-Space window across all taskbars or show every
window on the main display's taskbar only. This is a pure presentation choice:
physical display ownership remains unchanged for window commands.
`TaskbarOverflowLayout` subtracts fixed launcher, separator, spacing, and inset widths
from the live viewport, then shares the remainder equally across window buttons. It
first compresses from the selected preferred width to its selected minimum; only a
smaller viewport produces a scrollable document. Optional automatic-icon mode uses
the same calculation, switching labels to icons only when labeled minimums cannot fit
and returning them when space recovers. A hidden-scroller subclass preserves horizontal
deltas and translates predominantly vertical wheel input only while overflowing,
clamping the clip view to document bounds. The standard
...
Graphics window metadata, Foundation, and OSLog. It deliberately imports the one
private ApplicationServices symbol `_AXUIElementGetWindow`; that bridge is isolated
to AX enumeration and is used only to associate an AX window with public CG metadata.
It does not use private SkyLight or CGS Space APIs, Screen Recording,
ScreenCaptureKit, or window thumbnails. App Sandbox is disabled in the minimal
entitlements file: systemwide AX inspection/control is the core operation, and v1 is
intended for Developer ID distribution outside the Mac App Store. Hardened Runtime
is enabled by the Developer ID build script with no runtime exceptions.

## Reference research and techniques

The following projects were inspected for techniques, not copied architecture:

* [Switch](https://github.com/Sanyam-G/switch) demonstrates the AX-window-ID plus
  all-CG-window discovery technique used to recover minimized windows. TinyTaskbar's
  implementation was written independently and omits Switch's private CGS/Space
  subsystem.
...
* [AltTab](https://github.com/lwouis/alt-tab-macos) demonstrates combining AX titles
  and focus notifications with Core Graphics window metadata, while also
